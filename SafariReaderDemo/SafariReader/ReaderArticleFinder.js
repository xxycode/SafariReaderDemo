    ...
    if (LazyLoadRegex.test(t) && "function" == typeof URL) {
        ...
        if (m && m.search) {
            ...
            if (d && h) {
                ...
                if (!isNaN(parseInt(S)) && !isNaN(parseInt(C))) return m.searchParams.set(d, S),
...
function domDistance(e, t, n) {
    for (var r = [], i = e; i;) r.unshift(i),
    i = i.parentNode;
    var a = [];
    for (i = t; i;) a.unshift(i),
    i = i.parentNode;
    for (var o = Math.min(r.length, a.length), l = Math.abs(r.length - a.length), s = o; s >= 0 && r[s] !== a[s]; --s) if (l += 2, n && l >= n) return n;
    return l
}
function fontSizeFromComputedStyle(e, t) {
    var n = parseInt(e.fontSize);
    return isNaN(n) && (n = t || BaseFontSize),
    n
}
function contentTextStyleForNode(e, t) {
    ...
    for (var r = "descendant::text()[not(parent::h1) and not(parent::h2) and not(parent::h3) and not(parent::h4) and not(parent::h5) and not(parent::h6)]",
    i = e.evaluate(r, t, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null), a = i.snapshotLength, o = 0; o < a; ++o) {
        for (var l = i.snapshotItem(o), s = !1, c = l.parentElement; c !== t; c = c.parentElement) if (NegativeRegEx.test(c.className)) {
            s = !0;
            break
        }
        if (!s) {
...
function cachedElementBoundingRect(e) {
    ...
    return ReaderArticleFinderJS._elementsWithCachedBoundingRects.push(e),
    ReaderArticleFinderJS._cachedScrollX || ReaderArticleFinderJS._cachedScrollY ? (e._cachedElementBoundingRect = {
...
function clearCachedElementBoundingRects() {
    for (var e = ReaderArticleFinderJS._elementsWithCachedBoundingRects,
    t = e.length,
    n = 0; n < t; ++n) e[n]._cachedElementBoundingRect = null;
...
function levenshteinDistance(e, t) {
    for (var n = e.length,
    r = t.length,
    i = new Array(n + 1), a = 0; a < n + 1; ++a) i[a] = new Array(r + 1),
    i[a][0] = a;
    for (var o = 0; o < r + 1; ++o) i[0][o] = o;
    for (o = 1; o < r + 1; ++o) for (a = 1; a < n + 1; ++a) if (e[a - 1] === t[o - 1]) i[a][o] = i[a - 1][o - 1];
    else {
        var l = i[a - 1][o] + 1,
        s = i[a][o - 1] + 1,
        c = i[a - 1][o - 1] + 1;
...
function elementLooksLikeEmbeddedTweet(e) {
    ...
    return i && TweetIframeTitleRegex.test(i.getAttribute("data-iframe-title")) && ++n,
    e.classList.contains("twitter-tweet") && ++n,
    t.querySelector("[data-tweet-id]") && ++n,
    n > 2
}
...
function scoreMultiplierForElementTagNameAndAttributes(e) {
    ...
    n = e; n; n = n.parentElement) {
        ...
        "article" === normalizedElementTagName(n) && (t += ArticleMatchBonus)
    }
    return t < 0 ? 0 : t
}
function elementAtPoint(e, t, n) {
    if ("undefined" != typeof ReaderArticleFinderJSController && ReaderArticleFinderJSController.nodeAtPoint) {
        ...
        return r && r.nodeType !== Node.ELEMENT_NODE && (r = r.parentElement),
        r
    }
...
function elementFingerprintForDepth(e, t) {
    function n(e, t) {
        ...
        if (s && t > 0) {
            o.push(r);
            for (var c = 0; c < s; ++c) o.push(n(l[c], t - 1)),
            c !== s - 1 && o.push(a);
            o.push(i)
        }
        return o.join("")
...
function walkElementSubtree(e, t, n) {
    if (! (t < 0)) {
        ...
        i = r.length,
        a = t - 1,
        o = 0; o < i; ++o) walkElementSubtree(r[o], a, n);
        n(e, t)
    }
}
...
function getVisibleNonWhitespaceTextNodes(e, t, n, r, i) {
    function a(e) {
        var t = e.children[0];
        if (t) for (var n = t.children,
        r = n.length,
        i = 0; i < r; ++i) if ("none" !== getComputedStyle(n[i]).float) return ! 1;
        return ! 0
    }
    function o(e, r) {
    ...
    return r && (c.add("center"), c.add("font")),
    o(e, 0),
    s
}
function mapOfVisibleTextNodeComputedStyleReductionToNumberOfMatchingCharacters(e, t) {
    for (var n = {},
    r = getVisibleNonWhitespaceTextNodes(e, 100), i = r.length, a = 0; a < i; ++a) {
        var o = r[a],
        l = o.length,
        s = o.parentElement,
...
function nextNonFloatingVisibleElementSibling(e) {
    for (var t = e; t = t.nextElementSibling;) if (isElementVisible(t) && "none" === getComputedStyle(t).float) return t;
    return null
}
...
function hasClassMatchingRegexp(e, t) {
    for (var n = e.classList,
    r = n.length,
    i = 0; i < r; ++i) if (t.test(n[i])) return ! 0;
    return ! 1
}
...
function childrenWithParallelStructure(e) {
    ...
    i = 0; i < n; ++i) {
        var a = t[i];
        if (!SetOfCandidateTagNamesToIgnore.has(normalizedElementTagName(a)) && a.className) for (var o = a.classList,
        l = o.length,
        s = 0; s < l; ++s) { (m = r[u = o[s]]) ? m.push(a) : r[u] = [a]
        }
    }
...
CandidateElement.prototype = {
    calculateRawScore: function() {
        ...
        t = this.textNodes,
        n = t.length,
        r = 0; r < n; ++r) e += this.rawScoreForTextNode(t[r]);
        return e
    },
    ...
    depth: function() {
        return this.depthInDocument || (this.depthInDocument = elementDepth(this.element)),
        this.depthInDocument
    },
    finalScore: function() {
        return this.calculateLanguageScoreMultiplier(),
    ...
    scoreDensity: function() {
        ...
        h / g * 1e3 * (N / BaseFontSize) * this.languageScoreMultiplier
    },
    ...
    addTextNodesFromCandidateElement: function(e) {
        for (var t = this.textNodes.length,
        n = 0; n < t; ++n) this.textNodes[n].alreadyCounted = !0;
        var r = e.textNodes,
        i = r.length;
    ...
    rawScoreForTextNode: function(e) {
        ...
        for (var i = 1; r && r !== this.element;) i -= .1,
        r = r.parentNode;
    ...
    shouldDisqualifyDueToHeaderDensity: function() {
        ...
        if (n > 2) {
            for (var r = 0,
            i = cachedElementBoundingRect(this.element), a = i.height * PortionOfCandidateHeightToIgnoreForHeaderCheck, o = 0; o < n; ++o) {
    ...
    shouldDisqualifyDueToSimilarElements: function(e) {
        ...
        var n = function(e) {
            ...
            for (var n = e.parentElement; n && n !== this.contentDocument.body; n = n.parentElement) if (t.test(n.className)) return ! 0;
            return ! 1
        }.bind(this),
        r = this.element;
        if ("article" === normalizedElementTagName(r.parentElement)) return ! 1;
        let i = normalizedElementTagName(r);
        if ("li" === i || "dd" === i) for (var a = r.parentNode,
        o = a.children.length,
        l = 0; l < o; ++l) {
            ...
            for (l = 0; l < g; ++l) {
                ...
                if ((f = u[l]) !== r && (f.parentElement !== r && r.parentElement !== f && isElementVisible(f))) {
                    ...
                    if ((p || (p = new CandidateElement(f, this.contentDocument))) && p.basicScore() * ReaderMinimumAdvantage > this.basicScore()) {
                        ...
                        if (elementDepth(f) === d) for (; f.parentElement && r.parentElement && f.parentElement !== r.parentElement;) f = f.parentElement,
                        r = r.parentElement;
    ...
    shouldDisqualifyForDeepLinking: function() {
        ...
        r = pathComponentsForAnchor(n).length, i = [], a = t.getElementsByTagName("a"), o = a.length, l = 0; l < o; l++) {
            var s = a[l];
...
ReaderArticleFinder.prototype = {
    ...
    pointsForDominantIframeHitTest: function() {
        const e = 60;
        return [[e, e], [window.innerWidth - e, e], [e, window.innerHeight - e], [window.innerWidth - e, window.innerHeight - e]]
    },
    dominantContentIframe: function() {
        let e,
        t = this.pointsForDominantIframeHitTest(),
        n = t.length;
        for (let i = 0; i < n; i++) {
    ...
    nextPageURL: function() {
        if (!this._nextPageURL) {
            ...
            "undefined" != typeof ReaderArticleFinderJSController && e && (e = ReaderArticleFinderJSController.substituteURLForNextPageURL(e)),
            this._nextPageURL = e
        }
    ...
    containerElementsForMultiPageContent: function() {
        ...
        t = 3;
        for (var n, i = [], r = this.articleNode(), a = 0; ! (n = e.exec(r.getAttribute("id")));) if (! (r = r.parentElement) || a++===t) return [];
    ...
    adjustArticleNodeUpwardIfNecessary: function() {
        ...
        for (var t = e; t; t = t.parentElement) if (VeryPositiveClassNameRegEx.test(t.className)) return void(this.article.element = t);
        ...
        if (c) {
            ...
            "p" !== normalizedElementTagName(e) || e.className || (e = e.parentElement, d--),
            e.classList.length ? 1 === (m = elementsMatchingClassesInClassListIgnoringCommonLayoutClassNames(e.classList, this.contentDocument)).length && (m = elementsMatchingClassesInClassListIgnoringClassesWithNumericSuffix(e.classList, this.contentDocument)) : m = e.parentElement.children;
            for (var h = m.length,
            u = 0; u < h; ++u) {
                var g = m[u];
                if (e !== g && d === elementDepth(g) && (isElementVisible(g) && !g.querySelector("article") && Object.keys(e.dataset).join() === Object.keys(g.dataset).join() && dominantFontFamilyAndSizeForElement(e) === dominantFontFamilyAndSizeForElement(g))) return void(this.article.element = c)
            }
        }
        ...
        if (! (e = this.article.element).getAttribute("id") && e.className) {
            ...
            T = e.className,
            y = e.parentElement;
            if (y) for (var A = y.children,
            S = (u = 0, A.length); u < S; ++u) {
                var N = A[u];
    ...
    findArticleBySearchingQuirksList: function() {
        ...
        return findArticleNodeSelectorsInQuirksListForHostname(t.location.hostname, (function(n) {
            ...
            if (1 === i.length) return e = new CandidateElement(i[0], t),
            !0
        })),
        e
    },
    ...
    contentTextStyle: function() {
        return this._cachedContentTextStyle || (this._cachedContentTextStyle = contentTextStyleForNode(this.contentDocument, this.articleNode()), this._cachedContentTextStyle || (this._cachedContentTextStyle = getComputedStyle(this.articleNode()))),
        this._cachedContentTextStyle
    },
    commaCountIsLessThan: function(e, t) {
        for (var n = 0,
        i = e.textContent,
        r = -1; n < t && (r = i.indexOf(",", r + 1)) >= 0;) n++;
        return n < t
    },
    calculateLinkDensityForPruningElement: function(e, t) {
        var n = removeWhitespace(e.textContent).length;
        if (!n) return 0;
        for (var i = this.article.element,
        r = function() {
            for (var t = e.originalElement; t && t !== i; t = t.parentElement) if ("none" !== getComputedStyle(t).float) return t;
            return null
        } (), a = e.getElementsByTagName("a"), l = 0, o = a.length, s = 0; s < o; ++s) {
            var c = a[s]; ! r && c.href && t && t === dominantFontFamilyAndSizeForElement(c.originalElement) || (l += removeWhitespace(c.textContent).length)
        }
        return l / n
    },
    shouldPruneElement: function(e, t, n) {
        ...
        if ("canvas" !== s) {
            ...
            if (!c) {
                if ("p" === s) {
                    ...
                    f = e.nextSibling;
                    if (g && g.nodeType === Node.TEXT_NODE && !isNodeWhitespace(g) && f && f.nodeType === Node.TEXT_NODE && !isNodeWhitespace(f)) return ! 1
                }
                return ! 0
            }
            if ("p" === s) return ! 1
        ...
        if (t) {
            ...
            var E = t.className,
            v = t.getAttribute("id");
            PositiveRegEx.test(E) && p++,
            PositiveRegEx.test(v) && p++,
            NegativeRegEx.test(E) && p--,
            NegativeRegEx.test(v) && p--
        }
    ...
    newDivFromNode: function(e) {
        ...
        return e && (t.innerHTML = e.innerHTML),
        t
    },
    headerElement: function() {
        ...
        if (n && "header" === normalizedElementTagName(n) && !this.article.element.contains(n)) for (var i = n.querySelectorAll("img"), r = i.length, a = 0; a < r; ++a) {
            var l = i[a],
    ...
    adoptableLeadingMedia: function() {
        ...
        o = null,
        s = a.children.length;
        if ("div" === normalizedElementTagName(a) && s > 1 && s < t) for (var c = a.cloneNode(!0).querySelectorAll("p, div"), m = c.length, d = 0; d < m; ++d) {
            var h = c[d];
        ...
        g && u.setAttribute("src", g),
        !g && u.hasAttribute("src") || !u.hasAttribute("data-srcset") || u.setAttribute("srcset", u.getAttribute("data-srcset"));
    ...
    articleBoundingRect: function() {
        return this._articleBoundingRect || (this._articleBoundingRect = cachedElementBoundingRect(this.article.element)),
    ...
    adoptableArticle: function(e) {
        ...
        var m = !!s;
        if (m && i && (i === s && (m = !1), m)) {
            var d = i.compareDocumentPosition(s); (d & Node.DOCUMENT_POSITION_CONTAINS || d & Node.DOCUMENT_POSITION_CONTAINED_BY) && (m = !1)
        }
        if (m) {
    ...
    functionToPreventPruningElementDueToInvisibility: function() {
        return functionToPreventPruningDueToInvisibilityInQuirksListForHostname(this.contentDocument.location.hostname) ||
        function() {
            return ! 1
        }
    },
    cleanArticleNode: function(e, t, n, i) {
        ...
        function l() {
            ...
            if (a) {
                ...
                for (var l = 0; l < a; ++l) {
                    var o = r[l];
                    if ("none" === getComputedStyle(o).float) for (var s = o.children,
                    c = s.length,
                    m = 0; m < c; ++m) i.push(s[m]);
                    else i.push(o)
                }
            } else i = e.children;
            var d = i.length,
        ...
        function m(e, t) {
            for (var n = e[t]; n; n = n[t]) if (!isNodeWhitespace(n) && n.nodeType !== Node.COMMENT_NODE) return ! 1;
            return ! 0
        }
        ...
        for (var j = [], G = [], X = [], Y = [], K = []; b;) {
            try {
                ...
                if (C.originalElement = b, !N && elementAppearsToBeCollapsed(b) && (N = 1), (d.has($) || this.isAMPPage() && f.has($)) && (J = C), !J && b !== e && R.has(b) ? J = C: !J && b !== e && F.has(b) ? (C.parentElementBeforePruning = C.parentElement, J = C, j.push(C)) : elementIsAHeader(C) && previousLeafElementForElement(b) === I && C.classList.add("protected"), "twitter-widget" === $ && C.classList.add("protected"), !J && ("h1" === $ || "h2" === $)) if (b.offsetTop - e.offsetTop < HeaderMinimumDistanceFromArticleTop) {
                ...
                if (J || this.isMediaWikiPage() && /editsection|icon-edit|edit-page|mw-empty-elt/.test(b.className) && (J = C), "video" === $) if (C.getAttribute("src")) {
                    C.classList.add("protected");
                    var ie = cachedElementBoundingRect(b);
                    C.setAttribute("width", ie.width),
                    C.setAttribute("height", ie.height),
                ...
                let t = function() {
                    ...
                    for (let e of attributesForElement(b)) if (/^data-/.test(e.name) && LazyLoadRegex.test(e.value) && cachedElementBoundingRect(b).height) return ! 0;
                    return ! 1
                } ();
                if (!J && t && (!b.innerText || b.previousElementSibling && "noscript" === normalizedElementTagName(b.previousElementSibling))) if (Ie = lazyLoadingImageURLForElement(C, b.className)) {
                    var re = this.contentDocument.createElement("img");
                    re.setAttribute("src", Ie),
                    C.parentNode.replaceChild(re, C),
                    (C = re).originalElement = b,
                    $ = normalizedElementTagName(C),
                    J = C,
                    C.classList.add("protected")
                }
                ...
                if (!J && "div" === $ && C.parentNode) {
                    ...
                    if (le || ae.length ? elementIndicatesItIsASchemaDotOrgImageObject(b) && !C.querySelector("figure, .auxiliary") ? oe = "figure": ee && (oe = "span") : oe = "p", oe) {
                        for (var se = C.parentNode,
                        ce = this.contentDocument.createElement(oe); C.firstChild;) {
                            var me = C.firstChild;
                            ce.appendChild(me)
                        }
                        se.replaceChild(ce, C),
                        (C = ce).originalElement = b,
                        $ = normalizedElementTagName(C)
                    }
                }
                if (b.dataset && b.dataset.mathml && b.querySelector("math") && X.push(C), !J && C.parentNode && h.has($) && (C._originalElementDepthInCollapsedArea = N, E.push(C)), J || (isElementPositionedOffScreen(b) ? J = C: b === e || T || "none" === Q.float || B || !(cachedElementBoundingRect(b).height >= FloatMinimumHeight || b.childElementCount > 1) || (T = 1)), !J) {
                    if (sanitizeElementByRemovingAttributes(C), n === CleaningType.MetadataContent) if ("|" === C.innerText) C.innerText = "",
                    ...
                    if (A || "normal" === Q.fontStyle || (u.has($) || C.style && (C.style.fontStyle = Q.fontStyle), A = 1), !S && "normal" !== Q.fontWeight) {
                        if (!g.has($)) {
                            ...
                            Se = null;
                            isNaN(Ae) ? Se = Q.fontWeight: Ae <= 400 || Ae >= 500 && (Se = "bold"),
                            Se && C.style && (C.style.fontWeight = Se)
                        }
                        S = 1
                    }
                    if (T && "section" !== $ && s(b, e) || "aside" === $) {
                        ...
                        if (1 !== T || ee || (cachedElementBoundingRect(b).width <= MaximumFloatWidth ? C.setAttribute("class", "auxiliary float " + Q.float) : be || C.classList.add("auxiliary")), C.closest(".auxiliary") && b.style) {
                            ...
                            if ("table" === Q.display && /%/.test(xe) && parseInt(xe) < 2) C.style.width = Q.width;
                                ...
                                if (Ce) for (pe = Ce.length - 1; pe >= 0; --pe) {
                    ...
                    if ("table" === $) y || (y = 1);
                        ...
                        if (Ie = lazyLoadingImageURLForElement(C, b.className)) {
                            ...
                            if (!Me) {
                                ...
                                we = Le.length;
                                for (pe = 0; pe < we; ++pe) if (p.test(Le[pe].nodeName)) {
                                    Me = !0;
                                    break
                                }
                            }
                            Me && C.classList.add("protected"),
                            Z = !0
                        } ! Ie && C.hasAttribute("src") || !b.hasAttribute("data-srcset") || C.setAttribute("srcset", b.getAttribute("data-srcset")),
                    ...
                    } else if ("font" === $) C.removeAttribute("size"),
                    ...
                    C.removeAttribute("color");
                        ...
                        if (t === HTMLAnchorElement && "author" === b.getAttribute("itemprop")) C.classList.add("protected");
                            ...
                            if (!J) {
                                ...
                                C.parentNode.replaceChild(ce, C),
                                (C = ce).originalElement = b
                            }
                ...
                if (!J && elementIsCommentBlock(b) && (J = C), !J && z && cachedElementBoundingRect(b).top < z.top && VeryLiberalCommentRegex.test(b.className) && C.parentElement && (J = C), !J && "a" === $ && k.has(b.href)) {
                    for (var We, Ue, He = b,
                    Ve = C; (He = He.parentElement) && (Ve = Ve.parentElement);) {
                        const t = 10;
                        if (cachedElementBoundingRect(He).top - cachedElementBoundingRect(b).top > t) break;
                        if (He === e) break;
                        o(He) && (We = He, Ue = Ve)
                    }
                    We && (J = Ue, b = We, (C = Ue).originalElement = b, $ = normalizedElementTagName(C)),
                    He = null,
                    Ve = null,
                    We = null,
                    Ue = null
            ...
            if (!J || J.parentElement || i || (J = null), "div" === $ ? Y.push(C) : "aside" === $ && K.push(C), !J) {
                ...
                this._mapOfUniqueIDToOriginalElement.set(e, C.originalElement),
                C.setAttribute(this.elementReaderUniqueIDAttributeKey(), e),
                this._weakMapOfOriginalElementToUniqueID.set(C.originalElement, e)
            }
            ...
            r(1);
                ...
                for (var Ge; b !== e && !(Ge = b.nextElementSibling);) b = b.parentElement,
                C = C.parentElement,
                r( - 1);
                if (b === e) {
                    if (J && !elementIsProtected(J)) if (J.parentElement) J.remove();
        ...
        for (pe = E.length - 1; pe >= 0; --pe) { (vt = E[pe]).parentNode && this.shouldPruneElement(vt, vt.originalElement, U) && vt.remove(),
        ...
        for (pe = 0; pe < Je.length; ++pe) {
            ...
            if (!$e) {
                ...
                it = Ze.originalElement.querySelectorAll("embed, object").length; ! nt && it && ($e = !0)
            }
            if (!$e) {
        ...
        for (pe = 0; pe < dt; ++pe) {
            mt[ht = (ft = et[pe]).style.fontWeight] || (mt[ht] = []),
            mt[ht].push(ft)
        }
        for (var ht in mt) {
            var ut = mt[ht],
            gt = ut.length;
            if (gt > .7 * dt) for (pe = 0; pe < gt; ++pe) {
                var ft; (ft = ut[pe]).style.fontWeight = null,
        ...
        Et = pt.length;
        for (pe = 0; pe < Et; ++pe) {
            var vt; (vt = pt[pe]).classList.remove("protected"),
            vt.classList.length || vt.removeAttribute("class")
        }
        var Tt = t.querySelectorAll("p.auxiliary"),
        yt = Tt.length;
        for (pe = 0; pe < yt; ++pe) {
            for (var At = Tt[pe], St = [At], Nt = At.nextElementSibling; Nt && "p" === normalizedElementTagName(Nt) && Nt.classList.contains("auxiliary");) St.push(Nt),
        ...
        for (pe = 0; pe < It; ++pe) {
            ...
            wt = null,
            Rt = null;
            if (Lt) wt = depthOfElementWithinElement(Lt, t),
    ...
    leadingContentNodeWithSelector: function(e) {
        ...
        if (this._articleTitleElement) {
            let e,
            t = this.ancestorsOfElement(this.article.element);
            for (let n of t) if (n.contains(this._articleTitleElement)) {
                e = n;
                break
            }
        ...
        let l = Math.max(r, a);
        for (var o = this.article.element,
        s = 0; s < l && o.parentNode; ++s) {
            var c = (o = o.parentNode).querySelectorAll(e);
            for (var m of c) if (m && isElementVisible(m)) {
    ...
    mainImageNode: function() {
        ...
        if (e) return e;
        if (this.article && this.article.element) for (var t = this.article.element.querySelectorAll("img"), n = t.length, i = 0; i < n; ++i) {
            var r = t[i],
        ...
        try {
            for (var i = 0; i < n; ++i) {
                ...
                if ("https://schema.org" === l || "http://schema.org" === l) {
                    var o = a["@type"];
                    if (e.has(o)) return this._schemaDotOrgMetadataObjectForArticle = a,
                    a
                }
            }
            return null
    ...
    articleTitleInformation: function() {
        ...
        S = function() {
            for (var e = this.articleNode(); e; e = e.parentElement) if (elementIndicatesItIsASchemaDotOrgArticleContainer(e)) return e;
            return null
        }.bind(this)(),
        ...
        B = O.top,
        k = B; (this._articleWidth = O.width, this._leadingMediaElement = this.leadingImageNode(), this._leadingMediaElement || (this._leadingMediaElement = this.leadingVideoNode()), this._leadingMediaElement) && (k = (cachedElementBoundingRect(this._leadingMediaElement).top + B) / 2);
        ...
        for (V = 0; V < te; ++V) {
            ...
            if (I) {
                const e = t(I);
                n += e,
                e > StringSimilarityToDeclareStringsNearlyIdentical && re.push(he)
            }
            if (L) {
                const e = t(L);
                n += e,
                e > StringSimilarityToDeclareStringsNearlyIdentical && ae.push(he)
            }
            if (N) {
                const e = t(N);
                n += e,
                e > StringSimilarityToDeclareStringsNearlyIdentical && le.push(he)
            }
            if (b) {
                const e = t(b);
                n += e,
                e > StringSimilarityToDeclareStringsNearlyIdentical && oe.push(he)
            }
            if (w) {
                const e = t(w);
                n += e,
                e > StringSimilarityToDeclareStringsNearlyIdentical && se.push(he)
            }
            if (F) {
                const e = t(F);
                n += e,
                e > StringSimilarityToDeclareStringsNearlyIdentical && ce.push(he)
            }
        ...
        if (1 === re.length ? (Q = re[0]).headerText = trimmedInnerTextIgnoringTextTransform(Q) : 1 === ae.length ? (Q = ae[0]).headerText = trimmedInnerTextIgnoringTextTransform(Q) : 1 === le.length ? (Q = le[0]).headerText = trimmedInnerTextIgnoringTextTransform(Q) : 1 === se.length ? (Q = se[0]).headerText = trimmedInnerTextIgnoringTextTransform(Q) : 1 === ce.length && ((Q = ce[0]).headerText = trimmedInnerTextIgnoringTextTransform(Q)), !Q) for (V = 0; V < te; ++V) {
            he = W[V];
            ...
            if (Le) {
                ...
                if (we && we[0]) if (1 === this.contentDocument.querySelectorAll('*[property~="' + we[0] + '"]').length) { (Q = he).headerText = ue;
                    break
                }
            }
            if (!y.test(he.className)) {
                ...
                if (D && stringsAreNearlyIdentical(ue, D)) Me *= g;
                else if (e(ue, D)) Me *= f;
                ...
                if (ue !== M || !I) {
                    ...
                    Re || (Me *= Oe / BaseFontSize),
                    Me *= 1 + TitleCandidateDepthScoreMultiplier * elementDepth(he);
                    var Pe = parseInt(this.contentTextStyle().fontSize);
                    parseInt(Oe) > Pe * o && (Me *= s),
                    (c.test(he.className) || c.test(he.getAttribute("id"))) && (Me *= m);
                    var Be = he.parentElement;
                    Be && (c.test(Be.className) || c.test(Be.getAttribute("id"))) && (Me *= m),
                    -1 !== ie.indexOf(he) && (Me *= d);
                    _ = this.article.element;
                    for (var ke = he; ke && ke !== _; ke = ke.parentElement) if (SidebarRegex.test(ke.className)) {
                        Me *= p;
                        break
                    }
                    he.closest("li") && (Me *= E),
                    (!Q || Me > Q.headerScore) && ((Q = he).headerScore = Me, Q.headerText = ue)
                }
            }
        }
        var qe;
        ...
        return this._articleTitleInformation = je,
        je
    },
    ...
    elementAttributesContainImproperQuote: function(e) {
        for (var t = attributesForElement(e), n = t.length, i = 0; i < n; ++i) if (/['"]/.test(t[i].name)) return ! 0;
        return ! 1
    },
    articleSubhead: function() {
        ...
        if (v && r && r.contains(v) && v.innerText && v.innerText.trim() === r.innerText.trim() && (v = nextLeafElementForElement(v)), v && p.push(v), c.size) for (var T = a.querySelectorAll(HeaderElementsSelector + ", *[itemprop=description]"), y = T.length, A = 0; A < y; ++A) {
            var S = T[A];
        ...
        for (A = 0; A < N; ++A) {
            ...
            var M, L = !1;
            if (elementIsAHeader(b)) if (isNaN(o)) L = !0;
            else e(b) - 1 === o && (L = !0);
    ...
    adoptableMetadataBlock: function() {
        ...
        if (t && n) {
            ...
            i & Node.DOCUMENT_POSITION_CONTAINS && (t = null),
            i & Node.DOCUMENT_POSITION_CONTAINED_BY && (n = null),
            t === n && (n = null)
        }
        ...
        if (n) {
            ...
            e(s),
            s.innerText.trim() && (o = !0, s.classList.add("date"))
        }
    ...
    findArticleBylineElementWithoutRejection: function() {
        function e(e) {
            ...
            for (o = 0; o < r; ++o) {
                ...
                if (isElementVisible(s)) {
                    ...
                    E = a - f,
                    v = l - p,
                    T = Math.sqrt(E * E + v * v); (!u || T < h) && (u = s, h = T)
                }
            }
            return u
        }
        ...
        if (h) {
            if (h.matches(n) || h.innerText === o || (h = h.querySelector(n)), h) if (h.querySelector("li")) {
    ...
    findArticleDateElement: function() {
        function e(e) {
            for (var t = e; t && t !== l; t = t.parentElement) if (elementIsCommentBlock(t) || elementLooksLikeACarousel(t)) return ! 0;
            return ! 1
        }
        function t(t) {
            for (var n, i = null,
            ...
            a = 0; a < r; ++a) {
                ...
                if (isElementVisible(l) && !e(l)) {
                    ...
                    h = c - s,
                    u = m - d,
                    g = Math.sqrt(h * h + u * u); (!i || g < n) && (i = l, n = g)
                }
            }
            return i
        }
    ...
    updateArticleBylineAndDateElements: function() {
        ...
        t = this.findArticleDateElement(); ! t && e && (t = this.articleDateElementWithBylineElementHint(e)),
    ...
    findArticleBySearchingAllElements: function(e) {
        ...
        for (var i = this.highestScoringCandidateFromCandidates(n), r = i.element; r !== this.contentDocument; r = r.parentNode) if ("blockquote" === normalizedElementTagName(r)) {
    ...
    findExtraArticle: function() {
        if (!this.article) return null;
        for (var e = 0,
        t = this.article.element; e < 3 && t; ++e, t = t.parentNode) {
            var n = this.findExtraArticleCandidateElements(t);
            if (n && n.length) for (var i, r = this.sortCandidateElementsInDescendingScoreOrder(n), a = 0; a < r.length && ((i = r[a]) && i.basicScore()); a++) if (!i.shouldDisqualifyDueToScoreDensity() && !i.shouldDisqualifyDueToHorizontalRuleDensity() && !(i.shouldDisqualifyDueToHeaderDensity() || cachedElementBoundingRect(i.element).height < PrependedArticleCandidateMinimumHeight && cachedElementBoundingRect(this.article.element).width !== cachedElementBoundingRect(i.element).width)) {
    ...
    findExtraArticleCandidateElements: function(e) {
        ...
        for (var t = "preceding-sibling::*/descendant-or-self::*",
        n = this.contentDocument.evaluate(t, e, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null), i = n.snapshotLength, r = [], a = 0; a < i; ++a) {
    ...
    isMediaWikiPage: function() {
        return void 0 === this._isMediaWikiPage && (this._isMediaWikiPage = this.isGeneratedBy(/^MediaWiki /)),
    ...
    nextPageURLString: function() {
        ...
        for (var i = n,
        r = cachedElementBoundingRect(n).bottom + LinkMaxVerticalDistanceFromArticle; isElementNode(i) && cachedElementBoundingRect(i).bottom <= r;) i = i.parentNode;
    ...
    scoreNextPageLinkCandidate: function(e) {
        function t(e, t, n, i) {
            ...
            return (isNaN(a) || a >= MaximumExactIntegralValue) && (a = i),
            r === a ? n.lastInteger() === a + 1 : r === a + 1
        }
        function n(e) {
            for (var t = {},
            n = e.substring(1).split("&"), i = n.length, r = 0; r < i; ++r) {
                var a = n[r],
        ...
        var S = !1;
        if (e.search) for (var N in linkParameters = n(e.search), referenceParameters = n(i.search), linkParameters) {
            var b = linkParameters[N],
            x = N in referenceParameters ? referenceParameters[N] : null;
            if (x !== b) if (null === x && (x = ""), null === b && (b = ""), b.length < x.length) g++;
        ...
        S && (C += LinkURLSemanticMatchBonus),
        "li" === normalizedElementTagName(e.parentNode) && (C += LinkListItemBonus);
        p = e.innerText;
        return LinkNextMatchRegEx.test(p) && (C += LinkNextMatchBonus),
        LinkPageMatchRegEx.test(p) && (C += LinkPageMatchBonus),
        LinkContinueMatchRegEx.test(p) && (C += LinkContinueMatchBonus),
        C
    },
    elementContainsEnoughTextOfSameStyle: function(e, t, n) {
        ...
        for (var f in s) if (s[f] > o) return ! 0;
        return ! 1
    },
    ...
    pointsToUseForHitTesting: function() {
        ...
        return this.openGraphMetadataClaimsPageTypeIsArticle() && a.push([n - i, 1400]),
        a
    },
    findArticleByVisualExamination: function() {
        for (var e = new Set,
        t = this.pointsToUseForHitTesting(), n = t.length, i = AppleDotComAndSubdomainsRegex.test(this.contentDocument.location.hostname.toLowerCase()) ? 7200 : 1800, r = this.candidateElementFilter, a = 0; a < n; a++) for (var l = t[a][0], o = t[a][1], s = elementAtPoint(l, o, this.contentDocument); s && !e.has(s); s = s.parentElement) {
    ...
    findTextSamplesByVisualExamination: function() {
        ...
        function t(e) {
            const t = 10;
            let i = textContentAppearsToBeCJK(e, m) ? m: c,
            r = e.length;
            if (r < i) return null;
            if (r > g * s) return null;
            let a = n(e);
            return (a.match(/\n/g) || []).length > t ? null: a
        }
        function n(e) {
        ...
        function i(t, n) {
            let i = [],
            r = o.querySelectorAll(t);
            for (let t of r) {
                if (i.length >= n) break;
                if (elementDescendsFromElementInSet(t, a)) continue;
                let r = e(t);
                r && i.push([t, r])
            }
            return i
        ...
        p = t(f);
        p && (l.add(p), a.add(f));
        let E = this.pointsToUseForHitTesting(),
        v = E.length;
        for (var T = 0; T < v; T++) {
            let t = E[T][0],
            n = E[T][1],
            i = 0;
            for (let o = elementAtPoint(t, n, this.contentDocument); o && !r.has(o) && !(i > h); o = o.parentElement, i++) {
                let t = e(o);
                if (t) {
                    if (elementDescendsFromElementMatchingSelector(o, "code, form")) break;
                    l.add(t),
                    a.add(o);
                    break
                }
        ...
        for (let[e, t] of Object.entries(y)) {
            let n = i(e, t);
            for (let[e, t] of n) e && t && (l.add(t), a.add(e))
        }
        if (l.size < u) {
    ...
    findArticleFromMetadata: function(e) {
        ...
        if (1 === t.length) {
            var n = t[0];
            if (n.matches("article, *[itemprop=articleBody]")) if (o = CandidateElement.candidateIfElementIsViable(n, this.contentDocument, !0)) return e === FindArticleMode.ExistenceOfElement || o;
            var i = n.querySelectorAll("article, *[itemprop=articleBody]"),
            r = elementWithLargestAreaFromElements(i);
            if (r) if (o = CandidateElement.candidateIfElementIsViable(r, this.contentDocument, !0)) return e === FindArticleMode.ExistenceOfElement || o;
        ...
        if (this.openGraphMetadataClaimsPageTypeIsArticle() && !this.prismGenreClaimsPageIsHomepage()) {
            ...
            if (l) if (o = CandidateElement.candidateIfElementIsViable(l, this.contentDocument, !0)) return e === FindArticleMode.ExistenceOfElement || o;
            var o, s = this.contentDocument.querySelectorAll("article");
            if (1 === s.length) if (o = CandidateElement.candidateIfElementIsViable(s[0], this.contentDocument, !0)) return e === FindArticleMode.ExistenceOfElement || o
        }
        return null
    },
    articleTextContent: function() {
        return this._articleTextContent || this.adoptableArticle(),
        this._articleTextContent
    },
    ...
    articleTitleAndSiteNameFromTitleString: function(e) {
        ...
        for (var r, a, l = this.contentDocument.location.host.replace(/^(www|m|secure)\./, ""), o = l.replace(/\.(com|info|net|org|edu|gov)$/, "").toLowerCase(), s = 0; s < n; ++s) {
            ...
            if (2 === c.length) {
                ...
                p = Math.max(g, f); (!a || p > a) && (a = p, r = g > f ? {
                    siteName: m,
                    articleTitle: d
                }: {
                    siteName: d,
    ...
    pageInformation: function(e, t) {
        ...
        n || (n = userVisibleURLString(this.contentDocument.location.href)),
        n = n.trim(),
        e && (n = n.substring(0, e));
    ...
    pageTitleForTextAnalysis: function(e) {
        ...
        return n || (n = e["name:twitter:title"]),
        n || (n = e["name:sailthru.headline"]),
        n || (n = t.title),
        n
    },
    ...
    extractedArticleContent: function() {
        try {
            ...
            return ! n.author && r && (n.author = trimmedInnerTextIgnoringTextTransform(r)),
            n
        } catch(e) {
            let t = {};
            const n = e.message,
